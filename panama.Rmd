...
library(knitr) #Tablas e imagenes
library(readr) #Importar
library(dplyr) #Ordenar
library(stringr) #Tratar strings
library(rebus) #El patron
library(leaflet) #Mapas
library(ggplot2) #Graficos
#library(shiny) #Interactividad
```
...
nodes <- bind_rows(entity,intermediary,officer)


#Cleaning memory
rm(entity,intermediary,officer)

edges_net <- edges %>%
  filter(TYPE != "registered_address") %>%
  select(START_ID, END_ID, TYPE) %>%
  rename("type_edge" = "TYPE", "from" = "START_ID", "to" = "END_ID")

nodes_net <- rename(nodes, "id" = "node_id", "type_node"= "type")

rm(nodes_address, nodes_entity, nodes_intermediary, nodes_officer, edges, nodes)

library(GGally)
library(ggnetwork)
library(network)

net <- network(edges_net)

#, vertex.attr = nodes_net, directed = TRUE

ggnet2(net, size = 0.5, edge.alpha = 0.8, alpha = 0.75, edge.color = type_edge)

#, color = type_node, edge.color = type_edge
library(igraph)
g <- graph_from_data_frame(edges_net, nodes_net, directed = TRUE)
...
#  geom_edge_link(aes(alpha = type_edge)) + 
#  geom_node_point(aes(color = type_node))

#library(visNetwork)
#g_vis <- toVisNetworkData(g)

#visNetwork(nodes = g_vis$nodes, edges = g_vis$edges)
